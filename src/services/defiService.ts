...
const MOCK_PROTOCOLS: YieldProtocol[] = [
	{
		...
		riskLevel: 'low',
		description:
			'Earn rewards on USDC holdings with Coinbase. Backed by regulated institution.',
		isActive: true,
		totalTVL: 2500000000,
	...
	},
	{
		...
		riskLevel: 'low',
		description:
			'Supply USDC to Aave V3 lending protocol. Decentralized and battle-tested.',
		isActive: true,
		totalTVL: 8900000000,
	...
	},
	{
		...
		riskLevel: 'medium',
		description:
			'Mobile-first DeFi staking on Celo network. Higher yields, mobile optimized.',
		isActive: true,
		totalTVL: 450000000,
...
export class DeFiService {
	...
	static async stakeTokens(
		...
		if (amount < protocol.minimumAmount) {
			throw new Error(
				`Minimum amount is ${protocol.minimumAmount} ${tokenSymbol}`
			);
		}
		...
		setTimeout(async () => {
			...
			console.log(
				`[DeFiService] Staked ${amount} ${tokenSymbol} in ${protocol.name}`
			);
		}, 2000);
	...
	static getTotalYieldEarned(userAddress: string): number {
		...
		return positions.reduce(
			(total, position) => total + position.yieldEarned,
			0
		);
	}
	...
	static getTotalStaked(userAddress: string): number {
		const positions = this.getUserPositions(userAddress);
		return positions.reduce(
			(total, position) => total + position.principalAmount,
			0
		);
	}
		...
		switch (riskTolerance) {
			...
			case 'moderate':
				protocols = protocols.filter((p) =>
					['low', 'medium'].includes(p.riskLevel)
				);
				break;
			case 'aggressive':
