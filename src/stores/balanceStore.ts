...
export interface BalanceState {
	...
	invalidateBalance: (address: string, token?: 'SOL' | 'usdc') => void;
	invalidateAll: () => void;
	getBalance: (address: string, token: 'SOL' | 'usdc') => TokenBalance | null;
...
const createDefaultAddressBalances = (): AddressBalances => ({
	SOL: { ...createDefaultTokenBalance(), formatted: '0.0000' },
...
export const useBalanceStore = create<BalanceState>()(
	subscribeWithSelector((set, get) => ({
			...
			try {
				...
				set((state) => ({
					balances: {
						...
						[addressLower]: {
							SOL: {
								raw: balances.SOL.raw,
								formatted: balances.SOL.formatted,
								lastUpdated: now,
								isStale: false,
							},
							usdc: {
				...
				console.log(
					`[BalanceStore] Successfully fetched balances for ${address}`,
					balances
				);
			} catch (error) {
				console.error(
					`[BalanceStore] Error fetching balances for ${address}:`,
					error
				);
				...
				set((state) => ({
					balances: {
						...state.balances,
						[addressLower]: {
							...(state.balances[addressLower] ||
								createDefaultAddressBalances()),
							isLoading: false,
							error:
		...
		invalidateBalance: (address: string, token?: 'SOL' | 'usdc') => {
			const addressLower = address.toLowerCase();
			...
			console.log(
				`[BalanceStore] Invalidated ${token || 'all'} balance(s) for ${address}`
			);
		},
		...
		getBalance: (address: string, token: 'SOL' | 'usdc') => {
			const addressLower = address.toLowerCase();
			const balances = get().balances[addressLower];
			return balances?.[token] || null;
		},

		isBalanceStale: (address: string, maxAgeMs: number = 60000) => {
			// Default 1 minute
			const addressLower = address.toLowerCase();
			const balances = get().balances[addressLower];
			...
			const now = Date.now();
			const ethStale =
				balances.SOL.isStale || now - balances.SOL.lastUpdated > maxAgeMs;
			const usdcStale =
				balances.usdc.isStale || now - balances.usdc.lastUpdated > maxAgeMs;
...
export const useBalanceInvalidation = () => {
	...
	return {
		...
		onNetworkChange: () => {
			console.log(
				'[BalanceStore] Network change detected, invalidating all balances'
			);
			invalidateAll();
		},
		...
		onAppForeground: (address?: string) => {
			console.log(
				'[BalanceStore] App came to foreground, marking balances as stale'
			);
			if (address) {
				invalidateBalance(address);
			} else {
